...
PROSPECTOS_LOCALES_FILE = os.path.join(BASE_DIR, "prospectos_locales.json")
...
def _extraer_telefonos(texto: str, prefijo: str = "") -> List[str]:
    phones = []
    prefijo_clean = limpiar_telefono(prefijo) if prefijo else ""
    for m in re.finditer(r'(?:[\+]\d{1,3}[\s\-\.\(\)]*)?\d[\d\s\-\.\(\)]{6,}', texto):
        raw = m.group().strip()
        clean = limpiar_telefono(raw)
        if _es_telefono_valido(clean):
            if prefijo_clean:
                # Aceptar si empieza con el prefijo O si es local (empieza con 0)
                if not clean.startswith(prefijo_clean) and not clean.startswith("0"):
                    continue
            phones.append(raw)
    return phones
...
def _buscar_overpass(rubro: str, ubicacion: str) -> List[Dict]:
    ...
    for radio in [15000, 30000, 50000]:
        ...
        for ep in ["https://maps.mail.ru/osm/tools/overpass/api/interpreter", "https://overpass-api.de/api/interpreter"]:
            try:
                r = requests.post(ep, data={"data": q}, headers={"Accept": "application/json", "User-Agent": "PSKloudProspector/1.0"}, timeout=15)
...
def _ddg_text(query, region="wt-wt", max_results=5, timeout_sec=10):
...
def _buscar_ddg_batch(rubro: str, ubicacion: str, max_results: int = 30) -> List[Dict]:
    ...
    resultados, vistos = [], set()
    partes = [p.strip() for p in ubicacion.split(",")]
    ciudad = partes[0] if partes else ubicacion
    pais = partes[-1].strip() if len(partes) >= 2 else "Venezuela"
    # Construir "Ciudad, Pais" para queries más precisas
    ciudad_pais = f"{ciudad}, {pais}" if pais not in ciudad else ciudad
    prefijo = _detectar_prefijo(ubicacion)
    region = _detectar_region_ddg(ubicacion)

    queries = [
        f"{rubro} {ciudad_pais} telefono",
        f"{rubro} en {ciudad_pais} telefono contacto whatsapp",
        f"directorio {rubro} {ciudad_pais} telefono",
        f"guia {rubro} {ciudad_pais} numero",
        f"mejores {rubro} {ciudad_pais} contacto",
        f"{rubro} {ciudad} {pais} telefono email",
    ]
